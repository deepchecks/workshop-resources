...
def relate_column_to_label(column: pd.Series, label: pd.Series, label_power: float):
    return column * label.mean() * label_power
...
from deepchecks.tabular.datasets.regression.avocado import load_data

train, test = load_data(data_format='DataFrame')

test['region'] = insert_variants(test['region'], percent=10)
test['type'] = insert_categorical_drift(test['type'], percent=10, category='red')
test['Total Volume'] = insert_numerical_drift(test['Total Volume'], mean=100, std=100)
test = insert_duplicates(test, rows_to_duplicate_num=100, percent=10)
test['Total Bags'] = relate_column_to_label(test['Total Bags'], test['AveragePrice'], label_power=0.1)
